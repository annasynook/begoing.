...
export default function EventSheet({ C, event, session, onClose, requireAuth, onJoined }) {
  ...
  return (
    <div style={{
      ...
      <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 20, background: 'rgba(0,0,0,0.07)', border: 'none', borderRadius: '50%', width: 30, height: 30, fontSize: 16, color: '#666', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>x</button>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={share} style={{ ...actionBtn('rgba(255,255,255,0.7)', C.muted), flex: 'none', width: 64, padding: 0, fontSize: 14 }}>
          Share
        </button>
      </div>
    </div>
