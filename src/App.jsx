...
export default function App() {
  ...
  useEffect(() => {
    ...
    events.forEach((ev) => {
      ...
      if (markers.current[ev.id]) return
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (!pickMode) return
      setPickedLoc({ lng: +e.lngLat.lng.toFixed(6), lat: +e.lngLat.lat.toFixed(6) })
      setPickMode(false)
      canvas.style.cursor = ''
    }
  ...
  useEffect(() => {
    if (pickedLoc && !pickMode) setShowCreate(true)
  }, [pickedLoc, pickMode])
  ...
  const handleJoined = (updatedEvent) => {
    setEvents((es) => es.map((e) => e.id === updatedEvent.id ? updatedEvent : e))
    ...
    <div style={{ position: 'relative', width: '100%', height: '100dvh', overflow: 'hidden' }}>
      ...
      {pickMode && (
        <div style={{
          ...
          background: C.text, color: 'white', padding: '10px 20px',
          borderRadius: 30, fontSize: 14, fontWeight: 600, zIndex: 20,
          boxShadow: '0 4px 16px rgba(0,0,0,0.25)', whiteSpace: 'nowrap',
          display: 'flex', alignItems: 'center', gap: 10,
        }}>
          📍 Tap map to place event
          <button onClick={() => setPickMode(false)} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', borderRadius: 20, padding: '2px 10px', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, pointerEvents: 'auto' }}>
          {session ? (
            <UserMenu C={C} session={session} />
          ) : (
            <button onClick={() => setShowAuth(true)} style={btnStyle(C)}>
              Sign in
            </button>
          )}
        </div>
      ...
      {!pickMode && (
        <button
          ...
          style={{
            position: 'absolute', bottom: listOpen ? 'calc(45% + 16px)' : 96, right: 16,
      ...
      {!selected && (
        <button
          ...
          style={{
            position: 'absolute', bottom: listOpen ? '43%' : 16, left: '50%',
            transform: 'translateX(-50%)',
            background: C.surface, backdropFilter: 'blur(14px)',
            border: `1px solid ${C.border}`, borderRadius: 30,
            padding: '9px 20px', fontSize: 13, fontWeight: 600, color: C.text,
      ...
      {listOpen && !selected && (
        <div style={{
          ...
          background: C.surfaceBlur, backdropFilter: 'blur(16px)',
          ...
          {events.length === 0 ? (
            ...
            events.map((ev) => (
              <MiniCard key={ev.id} C={C} event={ev} onClick={() => {
                setSelected(ev)
                setListOpen(false)
                if (ev.lat && ev.lng) map.current.flyTo({ center: [ev.lng, ev.lat], zoom: 13, duration: 500 })
              }} />
            ))
          )}
        </div>
      ...
      {showCreate && (
        <CreateEventModal
          ...
          onClose={() => { setShowCreate(false); setPickedLoc(null) }}
        />
      )}
    </div>
  )
...
function UserMenu({ C, session }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} style={{ width: 34, height: 34, borderRadius: '50%', background: `linear-gradient(135deg, ${C.primary}, ${C.blue})`, border: 'none', color: 'white', fontWeight: 700, fontSize: 14, boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
        {initial}
      </button>
      {open && (
        <div style={{ position: 'absolute', top: 42, right: 0, background: 'white', borderRadius: 12, border: `1px solid ${C.border}`, boxShadow: '0 4px 20px rgba(0,0,0,0.12)', padding: 8, minWidth: 160, zIndex: 50 }}>
          ...
          <button onClick={async () => { await supabase.auth.signOut(); setOpen(false) }} style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: 'none', background: 'transparent', textAlign: 'left', fontSize: 14, color: C.text, cursor: 'pointer' }}>
            Sign out
          </button>
        </div>
      )}
...
function MiniCard({ C, event, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ display: 'flex', gap: 12, padding: '11px 0', borderBottom: `1px solid ${C.border}`, cursor: 'pointer', alignItems: 'center' }}>
      <div style={{ width: 42, height: 42, borderRadius: 12, background: `${C.primary}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 18 }}>◎</div>
